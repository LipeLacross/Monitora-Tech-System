...
// Função para amostrar dados (downsampling) se houver muitos pontos
function amostrarDados(data, labels, maxPoints = 100) {
    const sampledData = [];
    const sampledLabels = [];
...
async function renderChart(filter, date, startDate, endDate, month, year) {
    ...
    const movingAverage = calcularMediaMovel(data, 3);

    // Verifica se é necessário amostrar os dados para evitar problemas de performance
    const { sampledData, sampledLabels } = amostrarDados(data, labels, 100);
